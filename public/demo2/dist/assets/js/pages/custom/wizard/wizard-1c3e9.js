...
var KTWizard1 = function () {
	...
	var initWizard = function () {
			...
			validator.validate().then(function (status) {
				if (status == 'Valid') {
					if (wizard.getStep() == 1) {
					    ...
					    loadComponentDireksi();
					} else if (wizard.getStep() == 4) {
					    loadComponentDokumen();
					} else if (wizard.getStep() == 5) {
					    loadComponentPerizinan();
					} else {
						loadComponentPerusahaan();
					}
					_wizard.goNext();
					KTUtil.scrollTop();
				} else {
		...
		_wizard.on('change', function (wizard) {
			KTUtil.scrollTop();
			if ((wizard.getStep()-1) == 1) {
			    loadComponentNPWP();
			} else if ((wizard.getStep()-1) == 2) {
			    loadComponentSaham();
			} else if ((wizard.getStep()-1) == 3) {
			    loadComponentDireksi();
			} else if ((wizard.getStep()-1) == 4) {
			    loadComponentDokumen();
			} else if ((wizard.getStep()-1) == 5) {
			    loadComponentPerizinan();
			} else {
			    loadComponentPerusahaan();
			}
